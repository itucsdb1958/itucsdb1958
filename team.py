...
def team_schedule_page():
    ...
    schedule = select(
        ...
        table="schedule join member on schedule.member_id=member.id join person on person.id=member.person_id join team on team.id=person.team_id order by schedule.done,schedule.deadline",
